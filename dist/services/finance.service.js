...
export class FinanceService {
    static async createInvoice(deliveryId, narration, userId) {
        return await prisma.$transaction(async (tx) => {
            ...
            if (!delivery || !delivery.salesorder)
                throw new Error("Delivery or SO not found");
    ...
    static async createInvoiceFromGRN(grnId, narration, userId) {
        return prisma.$transaction(async (tx) => {
            ...
            await tx.supplierinvoiceline.createMany({
                data: grn.grnline.map((line) => {
                    const poLine = grn.purchaseorder?.purchaseorderline.find(p => p.po_line_id === line.po_line_id);
                    return {
    ...
    static async getInvoices(userId, statusFilter, search) {
        ...
        return dbInvoices.map((inv) => {
            ...
            const paid = inv.paymentallocation?.reduce((sum, p) => sum + Number(p.allocated_amount), 0) || 0;
    ...
    static async specificinvoice(id) {
        try {
    ...
    static async purchaseInvoiceList() {
        try {
            ...
            const formattedInvoices = invoices.map(inv => {
                const totalPaid = inv.paymentallocation.reduce((sum, alloc) => sum + Number(alloc.allocated_amount), 0);
                return {
                    id: inv.suppl_inv_id,
    ...
    static async specificPurchaseInvoice(invoiceId) {
        try {
            ...
            const totalPaid = invoice.paymentallocation.reduce((sum, alloc) => sum + Number(alloc.allocated_amount), 0);
    ...
    static async custinvoice(userid) {
    ...
    static async custpayment(userid) {
        const parties = await prisma.party.findMany({
            where: {
                user_id: userid
            },
    ...
    static async updateAccountBalances(tx, lines) {
    ...
    static async getLedger() {
        ...
        const accounts = await prisma.glaccount.findMany({
            include: {
                ...
                glaccount: { select: { name: true, gl_account_code: true } },
            },
    ...
    static async getPaymentAllocations() {
        ...
        return allocs.map(a => ({
            ...
            payment_number: a.payment.payment_number || `PMT-${a.payment_id}`,
    ...
    static async getAccountsDashboard() {
        const period = new Date().toISOString().slice(0, 7);
        const [arBal, apBal, cashBal, bankBal, recentJournals, pendingDeliveries, recentPayments] = await Promise.all([
            ...
            prisma.deliverynote.count({ where: { status: 'POSTED' } }),
    ...
    static async getPartyLedgerSummary(type) {
        const parties = await prisma.party.findMany({
            where: { type: type },
            include: {
                journalline: {
    ...
    static async getPartyLedgerDetails(partyId) {
        const lines = await prisma.journalline.findMany({
        ...
        const custInvMap = Object.fromEntries(custInvoices.map(i => [i.cust_inv_id, i.cust_invoice_number || `INV-${i.cust_inv_id}`]));
        const supplInvMap = Object.fromEntries(supplInvoices.map(i => [i.suppl_inv_id, i.suppl_invoice_number || `SINV-${i.suppl_inv_id}`]));
        const paymentMap = Object.fromEntries(payments.map(p => [p.payment_id, p.payment_number || `PAY-${p.payment_id}`]));
        let runningBalance = 0;
        return lines.map(l => {
